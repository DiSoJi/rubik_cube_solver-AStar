...
def permutaciones(abierto):
    ...
    for movimiento in movimientos:
        ...
        if(str(cuboNuevo) not in explorados):
            nota= (22-movs)*300 + medir(cuboNuevo)
            explorados[str(cuboNuevo)]=1
            if(nota>notaMaxima):
                notaMaxima=nota
                seleccionado=len(abiertos)
            abiertos+=[Cubo(cuboNuevo,abierto.listaMovimientos+[i],idCubo,nota)]
            
        if(str(cuboNuevoR)not in explorados):
            nota= (22-movs)*300 + medir(cuboNuevoR)
            explorados[str(cuboNuevoR)]=1
            if(nota>notaMaxima):
                notaMaxima=nota
                seleccionado=len(abiertos)
            abiertos+=[Cubo(cuboNuevoR,abierto.listaMovimientos+[i],idCubo,nota)]
        i+=1
...
def medir(cubo):
    ...
    for i in range(0,len(meta)):
        ...
        #pertenencia a cara
        if(cubo[i]-1==centro or cubo[i]-2==centro or cubo[i]-3==centro or cubo[i]-4==centro or cubo[i]+4==centro or cubo[i]+3>centro or cubo[i]+2>centro or cubo[i]+1>centro):#en el rango del centro, por eso tanto if
            r+=3
            bonusCara+=1
        ...
        #si tiene toda una cara valen doble, genial
        if(bonusCara==8):
            r+=45
        if (cubo[i]==meta[i]):
            r+=5
