...
cuboPrueba=[24, 30, 27, 48, 4, 7, 26, 32, 29, 33, 39, 36, 1, 13, 5, 35, 41, 38, 0, 10, 2,
            19, 22, 25, 6, 16, 8, 20, 23, 17, 50, 31, 52, 18,
            21, 15, 47, 43, 53, 34, 40, 28, 45, 37, 51, 42, 12, 9, 3, 49, 46, 44, 14, 11]

##FUNCIONES DE SOLUCION
def iniciar(cubo):
    global abiertos,cerrados,seleccionado,notaMaxima,idCubo,explorados
    abiertos=[Cubo(cubo,[],0,0)]
    cerrados=[]
    seleccionado=0
    notaMaxima=0
    idCubo=1
    ...
    resuelto=main()
    time2 = time.time()
    return resuelto.listaMovimientos
...
def aplicarMovimiento(cubo,indice):
    global movimientos
    r=copy.copy(cubo)
    cero=0
    uno=1
...
def randomCube(cubo,cantidadMovimientos):
    ...
    while(cantidadMovimientos>0):
        ...
        cubo=aplicarMovimiento(cubo,selected+1)
       # print(selected)
