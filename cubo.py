...
# Edited and reworked by: Diego Solís Jiménez
# Adapted from cube code written by David Hogg, under GNU License (Open Source)
...
class InteractiveCube(plt.Axes):
    def __init__(self, cube=None,
        ...
        self.mov_index = 0
        self.prev_index = 0
        self.cubo_org = [ 0,1 ,2 ,3 ,4 ,5 ,6 ,7 , 8,
    ...
    def _initialize_widgets(self):
        ...
        self._btn_reset = widgets.Button(self._ax_reset, 'Vista Inicial')
        self._btn_reset.on_clicked(self._reset_view)
        
        self._ax_solve = self.figure.add_axes([0.55, 0.05, 0.2, 0.075])
        self._btn_solve = widgets.Button(self._ax_solve, 'Calcular Solución')
        self._btn_solve.on_clicked(self._desordenar_cubo)
        
        self._ax_test = self.figure.add_axes([0.35, 0.05, 0.2, 0.075])
        self._btn_test = widgets.Button(self._ax_test, 'Auto Ordenar')
        self._btn_test.on_clicked(self._ordenar_cubo_aut)

        self._ax_nextMv = self.figure.add_axes([0.55,0.125,0.2,0.075])
        self._btn_nextMv = widgets.Button(self._ax_nextMv, 'Mov Siguiente')
        self._btn_nextMv.on_clicked(self._mov_sig)

        self._ax_prevMv = self.figure.add_axes([0.35,0.125,0.2,0.075])
        self._btn_prevMv = widgets.Button(self._ax_prevMv, 'Mov Anterior')
        self._btn_prevMv.on_clicked(self._mov_ant)
    ...
    def _key_press(self, event):
        ...
        if event.key == 'shift':
            ...
            self.deordering_movs.append(direction * (self.movList.index(event.key.upper())+1))
    ...
    # Aca se lee la lista de movimientos para desordenar y se procede a dejar el cubo
    # en un estado inicial
    def _desordenar_cubo(self,event):
        #movDesorden=rubik4.obtenerCuboDesordenado()
        cubo_to_solve = copy.copy(self.cubo_org)
        #movDesorden = [1,6,5,-2]
        #self.solve_movs = [2,-5,-6,-1]
        """
        L: Vertical Izquierda
        R: Vertical Derecha
        U: Horizontal Arriba
        ...
        B: Rotación Atras
        """
        for mov in self.deordering_movs:
            cubo_to_solve = copy.copy(rubik4.aplicarMovimiento(cubo_to_solve, -1*mov))
        
        self.solve_movs = copy.copy(rubik4.iniciar(cubo_to_solve))
        self.next_movs = copy.copy(self.solve_movs)

        for i in self.solve_movs:
            temp = i*-1
            self.prev_movs.insert(0,temp)
        """
        print("Movimientos")
        print(self.deordering_movs)
        print(self.solve_movs)
        print(self.prev_movs)
        print(self.next_movs)"""

        self.deordering_movs = []
        self.mov_index = 0
        self.prev_index = len(self.prev_movs)
    
    def _ordenar_cubo_aut(self,event):
        while self.mov_index < len(self.next_movs):
            mov = self.next_movs[self.mov_index]
            if mov < 0:
                mov = (-1 * mov)-1
                c.rotate_face(self.movList[mov],-1)
            else:
                mov -= 1
                c.rotate_face(self.movList[mov],1)
            self._draw_cube()
            ...
            plt.pause(0.8)
        self.prev_index = 0
        self.mov_index = len(self.next_movs)
        self.solve_movs = []
        self.deordering_movs = []
    
    def _mov_sig(self,event): 
        
        if (self.mov_index >= len(self.next_movs or self.mov_index < 0)):
            print("out of bonds")
            self.solve_movs = []
        else:
            mov = self.next_movs[self.mov_index]
        ...
        print(self.mov_index)"""
        self._draw_cube()

    def _mov_ant(self,event): #Test if this is working. 
        
        if (self.prev_index < 0 or self.prev_index >= len(self.prev_movs)):
            print("out of bonds")
        else:
            print(self.prev_index)
            mov = self.prev_movs[self.prev_index]
            if mov < 0:
                mov = (-1 * mov)-1
                c.rotate_face(self.movList[mov],-1)
